		...
		if (accessor.getBlockState().is(TFBlocks.CHISELED_CANOPY_BOOKSHELF) && accessor.getBlockState().getValue(ChiseledCanopyShelfBlock.SPAWNER)) {
			if (accessor.getPlayer().isCreative()) {
				...
				if (te instanceof ChiseledCanopyShelfBlockEntity shelf) {
					...
					if (logic != null && shelf.getLevel() != null) {
						ValueInput input = TagValueInput.create(
							ProblemReporter.DISCARDING,
							shelf.getLevel().registryAccess(),
							logic.entityToSpawn()
						);
						EntityType.by(input).ifPresent(type -> tooltip.replace(Identifier.fromNamespaceAndPath("jade", "object_name"), IThemeHelper.get().title(Component.translatable("jade.spawner", accessor.getBlock().getName().getString(), type.getDescription().getString()))));
					}
				}
			}
		}
