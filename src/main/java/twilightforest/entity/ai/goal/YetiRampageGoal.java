...
public class YetiRampageGoal extends Goal {
	...
	public void tick() {
		...
		this.yeti.destroyBlocksInAABB(Goal.getServerLevel(this.yeti), this.yeti.getBoundingBox().inflate(1, 2, 1).move(0, 2, 0));

		// regular falling blocks, twice a second
		if (this.currentDuration % 10 == 0) {
			this.yeti.makeRandomBlockFall(Goal.getServerLevel(this.yeti), 30, 80);
		}
		...
		if (this.currentDuration < 40 && this.currentDuration % 10 == 0) {
			this.yeti.makeRandomBlockFall(Goal.getServerLevel(this.yeti), 15, 40);
		}
