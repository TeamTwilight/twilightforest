...
public class FogHandler {
	...
	protected static void renderFog(ViewportEvent.RenderFog event) {
		// TODO: 26.1.2 - Verify FogType.NONE works correctly with new FogEnvironment system
		if (event.getType().equals(FogType.NONE) && Minecraft.getInstance().getCameraEntity() instanceof LocalPlayer player && player.level() instanceof ClientLevel clientLevel && clientLevel.dimension().identifier().getNamespace().equals(TwilightForestMod.ID)) {
			if (SKY_CHUNK_LOADED) {
		...
		} else if (Minecraft.getInstance().getCameraEntity() instanceof LocalPlayer camPlayer && camPlayer.level() instanceof ClientLevel camLevel) {
			if (TERRAIN_CHUNK_LOADED) {
				boolean spooky = isSpooky(camLevel, camPlayer);
			...
			} else if (SKY_CHUNK_LOADED || camLevel.isLoaded(camPlayer.blockPosition())) { //SKY is always called first in vanilla, so we only need to check if the SKY flag is true, but just in case
				TERRAIN_CHUNK_LOADED = true;
				TERRAIN_FAR = isSpooky(camLevel, camPlayer) ? event.getFarPlaneDistance() * 0.5F : event.getFarPlaneDistance();
				TERRAIN_NEAR = isSpooky(camLevel, camPlayer) ? TERRAIN_FAR * 0.75F : event.getNearPlaneDistance();
			}
		}
	}
