...
public class RedThreadRenderer implements BlockEntityRenderer<RedThreadBlockEntity, RedThreadRenderState> {
	...
	public void extractRenderState(RedThreadBlockEntity blockEntity, RedThreadRenderState state, float partialTicks, Vec3 cameraPosition, ModelFeatureRenderer.@Nullable CrumblingOverlay breakProgress) {
		...
		if (player != null) {
			boolean wearsActivatedTravellersGoggles = player.getData(TFDataAttachments.TRAVELLERS_GOGGLES_RED_THREAD_VISION) && TravellersModifiersManager.isModifierActive(player, TravellersModifiersManager.lookupHolder(player.registryAccess(), TravellersModifiersManager.RED_THREAD_VISION_MODIFIER).orElseThrow());
