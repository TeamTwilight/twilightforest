...
public class RedThreadRenderer<T extends RedThreadBlockEntity> implements BlockEntityRenderer<T> {
	...
	public void render(T entity, float ticks, PoseStack stack, MultiBufferSource buffer, int light, int overlay) {
		...
		boolean wearsActivatedTravellersGoggles = player.getData(TFDataAttachments.TRAVELLERS_GOGGLES_RED_THREAD_VISION) && TravellersModifiersManager.isModifierActive(player.getItemBySlot(EquipmentSlot.HEAD), TravellersModifiersManager.RED_THREAD_VISION_MODIFIER);
