...
public class BlockstateGenerator extends BlockModelBuilders {
	...
	private void registerBush(Block block) {
		...
		for (int age = 0; age <= TFBushBlock.MAX_AGE; age++) {
			...
			for (int snow = SnowLoggable.MIN_SNOW_LAYERS; snow <= SnowLoggable.MAX_SNOW_LAYERS; snow++) {
				BUSH_MODELS[age][snow] = createBushModel(block, age, snow, texture);
			}
		}
	...
	private ModelFile createBushModel(Block block, int age, int snowLayers, ResourceLocation texturePath) {
		...
		addBushElements(builder, age, snowHeight);
	...
	private void addBushElements(ModelBuilder<BlockModelBuilder> b, int age, float snowHeight) {
		switch (age) {
			case 0 -> addShrunkBox(b,  4,  0,  4, 12,  8, 12, snowHeight, (direction, faceBuilder) -> faceBuilder.texture("#all"));
			case 1 -> addShrunkBox(b,  2,  0,  2, 14, 12, 14, snowHeight, (direction, faceBuilder) -> faceBuilder.texture("#all"));
			case 2 -> addShrunkBox(b,  0,  0,  0, 16, 16, 16, snowHeight, (direction, faceBuilder) -> faceBuilder.texture("#all"));
			case 3 -> addShrunkBox(b,  0,  0,  0, 16, 16, 16, snowHeight,
